...
class Cityscapes_Pre(SegmentationList):
    CLASSES = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light', 'traffic sign',
               'vegetation', 'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle',
               'bicycle']
    ...
    def __init__(self, root, split='train', data_folder='leftImg8bit', label_folder='gtFine', pre_transforms=None ,**kwargs):
        assert split in ['train', 'val']
        ...
        data_list_file = os.path.join(root, "image_list", "{}.txt".format(split))
        self.split = split
        super(Cityscapes_Pre, self).__init__(root, Cityscapes.CLASSES, data_list_file, data_list_file,
                                         os.path.join(data_folder, split), os.path.join(label_folder, split),
                                         id_to_train_id=Cityscapes.ID_TO_TRAIN_ID,
                                         train_id_to_color=Cityscapes.TRAIN_ID_TO_COLOR, **kwargs)
        self.invariant_transforms = pre_transforms
    ...
    def __getitem__(self, index):
        ...
        image, label = self.invariant_transforms(image, label)
